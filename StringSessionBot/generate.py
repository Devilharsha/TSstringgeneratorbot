...
async def generate_session(bot, msg, telethon=False):
    ...
    text = "**{} STRING SESSION** \n\n`{}` \n\nGenerated by @TSstringgeneratorbot".format("TELETHON" if telethon else "PYROGRAM", string_session)
    try:
        await client.send_message("me", text)
    except KeyError:
        pass
    await client.disconnect()
    await phone_code_msg.reply("Successfully generated {} string session. \n\nPlease check your saved messages! \n\nBy @Tsstringgeneratorbot".format("telethon" if telethon else "pyrogram"))
